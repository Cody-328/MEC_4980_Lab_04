...
void loop() {
  ...
  case TwoAxis: 
    ...
    if (psi > 0.0) {
      ...
      drawTriangle(32, 47, -1, 1, true);
    }
    
    break;
  case XAxis: 
     theta = getXangle();
    sprintf(pout, "X:%d degs", (int)theta);
    myOLED.text(4, 20, pout);
  ...
  case YAxis: 
    psi = getYangle();
    sprintf(pout, "Y:%d degs", (int)psi);
    myOLED.text(4, 38, pout);
